...
claude plugin install modelence@modelence --scope project
```
...
Modelence Cloud also hosts projects that don't use the framework — an Express API, a Vite site, a
Next.js app. How such a project is built and run is described in a `modelence.config.json` file at
its root, and your agent is the right one to write it, since it knows the codebase. Give it the
[Deploy Setup](/deploy/setup) page — it is written for agents and covers the file's reference, worked
examples, and how to verify the result locally — then run `npx modelence deploy`:
