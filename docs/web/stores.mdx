...
// Atomic find-or-create — returns the document and whether it was just created
const { doc, isNew } = await dbTodos.findOneAndUpsert(
  { userId: user.id, title: 'Unique task' },
  { $setOnInsert: { userId: user.id, title: 'Unique task', isCompleted: false } }
);
if (isNew) {
  // ...run first-time setup for the new document
}
