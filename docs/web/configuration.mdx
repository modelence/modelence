...
`createClientModule` also provides typed wrappers for queries and mutations,
compatible with TanStack Query's `useQuery` and `useMutation`. The argument and
return types are inferred from the handler signatures — no manual type parameters needed.
...
function AdminPanel() {
  ...
  // invite({ email: '...' }) — args are typed as { email: string }
...
- [`createClientModule`](/api-reference/modelence/client/functions/createClientModule) — Create a typed client accessor for a module
