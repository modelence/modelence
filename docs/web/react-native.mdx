...
reads its own storage first and asks its opener only when that is empty, so a
...
<Warning>
  ...
  `loginWithOAuth` has nothing to ask and fails in the partitioned iframe case
  as before.
</Warning>
