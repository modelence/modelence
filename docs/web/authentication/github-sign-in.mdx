...
The easiest way to add GitHub Sign-In is using the `@modelence/auth-ui` package, which provides a complete `LoginForm` component with built-in GitHub authentication support:
...
The `LoginForm` component automatically displays a "Sign in with GitHub" button when GitHub authentication is enabled from the **Authentication → GitHub** section in [cloud.modelence.com](https://cloud.modelence.com) or via the `MODELENCE_AUTH_GITHUB_ENABLED` environment variable. The component includes:
- Pre-styled button with GitHub branding and icon
...
```typescript
{
  ...
  name?: string;           // Full name from GitHub profile
  picture?: string;        // Avatar URL from GitHub
}
```
...
If you prefer not to use `@modelence/auth-ui`, you can manually implement GitHub Sign-In by redirecting to the authentication endpoint:
