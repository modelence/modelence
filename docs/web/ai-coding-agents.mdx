...
```json .claude/settings.json
{
  ...
  "enabledPlugins": ["modelence@modelence"]
}
```
...
<Note>
If `/plugin` isn't recognized, update Claude Code by running `claude update`, then start a new session.
</Note>
...
## Inspecting a cloud environment

Modelence Cloud also exposes an MCP server at `https://cloud.modelence.com/mcp` for inspecting your
environments — app status, logs, and database contents — covering both deployed cloud environments and
App Builder sandboxes. It requires signing in, so consent happens in the browser the first time you
connect:

```bash
claude mcp add --transport http modelence-cloud https://cloud.modelence.com/mcp
```

<Note>
This server inspects the **environment** — status, logs, and data. Your local files stay the source of
truth for code while you're working locally.
</Note>
