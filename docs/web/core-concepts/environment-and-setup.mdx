...
| `MODELENCE_AUTH_MOBILE_REDIRECT_URLS` | string (comma-separated) | — | `packages/modelence/auth/providers/mobileRedirect.ts` (deep links native OAuth may return to, e.g. `myapp://auth`; merged with `auth.mobile.redirectUrls`) |
