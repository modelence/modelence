...
## Handling Duplicate Key Index Conflicts

If index creation fails (for example, because duplicate records exist), you can provide store-specific recovery logic with `deduplicateIndexes`.
...
export const locksCollection = new Store('_modelenceLocks', {
  ...
  // Wait for this store's indexes before continuing app startup
...
This callback is only used when index creation fails, and only for stores that define it.
