...
Access the underlying MongoDB collection for advanced operations:

```typescript
const collection = dbTodos.rawCollection();
const database = dbTodos.getDatabase();

// Use native MongoDB driver methods
await collection.createIndex({ customField: 1 });
```

### Migrations

Rename collections during migrations:

```typescript
// Rename 'old_todos' collection to 'todos'
await dbTodos.renameFrom('old_todos');
```
