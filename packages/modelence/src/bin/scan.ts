...
/**
 * Generates the content for the typed client modules file.
 * Each auto-loaded module gets a typed `createClientModule` export so clients
 * can import from `@modelence/modules` with full autocomplete and type inference.
 */
export function generateClientModulesContent(modules: ScannedModule[]): string {
  const modulesWithMethods = modules.filter(
    (mod) => mod.folders.queries?.length || mod.folders.mutations?.length
  );

  if (modulesWithMethods.length === 0) {
    return '';
  }
  ...
  for (const mod of modulesWithMethods) {
    ...
    for (const folderName of ['queries', 'mutations'] as const) {
      ...
      for (const file of files) {
        const entryId = toIdentifier(file.name);
        const importId = `${mod.identifier}_${folderName}_${entryId}`;
        const importPath = toRelativeImport(file.filePath);
        lines.push(`import type ${importId} from '${importPath}';`);
      }
    }
  }
  ...
  // Generate a createClientModule export for each module
  for (const mod of modulesWithMethods) {
    ...
    for (const file of queryFiles) {
      const entryId = toIdentifier(file.name);
      lines.push(`    ${JSON.stringify(file.name)}: typeof ${id}_queries_${entryId};`);
    }
    lines.push(`  };`);

    // mutations
    const mutationFiles = mod.folders.mutations ?? [];
    lines.push(`  mutations: {`);
    for (const file of mutationFiles) {
      const entryId = toIdentifier(file.name);
      lines.push(`    ${JSON.stringify(file.name)}: typeof ${id}_mutations_${entryId};`);
    }
    lines.push(`  };`);

    lines.push(`}>(${JSON.stringify(mod.name)});`);
    lines.push('');
  }
...
/**
 * Ensures the user's tsconfig.json has a `@modelence/modules` path alias
 * pointing to the generated client modules file. Adds it once; skips if present.
 */
function ensureTsconfigModulesAlias(): void {
  ...
  try {
    ...
    console.warn(
      'Could not parse tsconfig.json (it may contain comments). ' +
        'Please add the following path alias manually:\n' +
        '  "paths": { "@modelence/modules": ["./.modelence/generated/clientModules"] }'
    );
    return;
  }
  ...
  console.log('Added @modelence/modules path alias to tsconfig.json');
}
...
export async function generateAutoLoadFiles(serverDir: string, serverEntry: string): Promise<void> {
  ...
  const clientModulesContent = generateClientModulesContent(modules);
  const clientModulesPath = path.join(generatedDir, 'clientModules.ts');
  if (clientModulesContent) {
    fs.writeFileSync(clientModulesPath, clientModulesContent);
    ensureTsconfigModulesAlias();
  } else if (fs.existsSync(clientModulesPath)) {
    fs.unlinkSync(clientModulesPath);
  }
