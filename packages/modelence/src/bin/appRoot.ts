...
// The archive remains rooted at cwd; build.root only picks the directory
...
export async function resolveAppRoot(cwd: string, root = '.'): Promise<string> {
  ...
  if (!isInside(archiveRoot, candidate)) {
    throw new Error('build.root must stay inside the project.');
  }
  let appRoot: string;
  try {
    appRoot = await fs.realpath(candidate);
  } catch {
    throw new Error(`build.root "${root}" does not exist.`);
  }
