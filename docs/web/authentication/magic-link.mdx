...
Magic link works as a combined sign-in and sign-up: if the email is not registered yet, the account is created automatically when the link or code is used — just like OAuth sign-in.
...
3. An email with the sign-in link and the code is sent to the address — whether or not an account exists, so the response never reveals whether an email is registered
