...
If a unique index cannot be created because duplicate records already exist, you can provide store-specific recovery logic with `deduplicateIndexes`.
...
This callback is only used when index creation hits duplicate-key conflicts, and only for stores that define it.
