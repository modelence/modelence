...
- The `modelence` MCP server needs a one-time sign-in. If its tools are missing or it reports it needs authentication, tell the user how to sign in where they are: in the Claude Code terminal or an IDE extension, run `/mcp`, pick `modelence`, and finish in the browser; in the Claude desktop app, click **+** next to the prompt box → **Plugins** → **Modelence** → **Connect**. Never ask them for a token.
