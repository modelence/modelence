...
The code is also bound to the device that started the flow. `signInWithOAuth` keeps a verifier in memory and `loginWithOAuth` replays it, so a `myapp://auth?code=...` link handed to your app from anywhere else is rejected before the code is ever sent. This means `loginWithOAuth` must be called from the same app process that called `signInWithOAuth` — if the app is killed mid-flow, start the sign-in again.
