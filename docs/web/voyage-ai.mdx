...
5. **Add your Voyage AI API key**:
   ...
   - In the [Modelence Cloud dashboard](https://cloud.modelence.com/), open your environment, click the **Configs** tab, and set `voyage.apiKey` to your API key. The value is stored encrypted and delivered to your app at runtime.
...
export function getVoyageClient() {
  if (!voyageClient) {
    const apiKey = getConfig('voyage.apiKey') as string;
    if (!apiKey) {
      throw new Error('MODELENCE_VOYAGEAI_API_KEY is not set');
    }
...
Set your Voyage AI API key from the [Modelence Cloud dashboard](https://cloud.modelence.com/):

1. Open your environment
2. Click the **Configs** tab
3. Add (or edit) the `voyage.apiKey` config and paste your API key as the value
4. Save — the new value takes effect on the next server boot

This matches the `apiKey` entry declared in the module's `configSchema` in Step 4 (which the `voyage` module namespace prefixes to `voyage.apiKey`). Because it is declared as `type: 'secret'`, the value is stored encrypted, never sent to the client, and masked in the dashboard. At runtime, the `getConfig('voyage.apiKey')` call in `voyage.ts` reads it transparently.

<Tip>
Rotating the key later? Just update the same `voyage.apiKey` config in the dashboard — no code changes or redeploys needed.
</Tip>
