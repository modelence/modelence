...
Migrations let you evolve existing MongoDB data safely when your schema or
...
- `handler` - Async function that performs the data change
...
- Stores using `indexCreationMode: 'blocking'` are awaited before migrations begin.
- Stores using `indexCreationMode: 'background'` may still be creating indexes while migrations run.
