...
2. **Configure Application Details**
   ...
   <Note>
     GitHub allows only one callback URL per OAuth App, and that is all Modelence
     needs. React Native sign-in reuses this same callback and returns to your app
     through a deep link — see [React Native](/react-native#oauth-sign-in).
   </Note>
...
The redirect flow above assumes your app can navigate in place. If your app is
embedded in a **cross-origin iframe** — an embedded preview, a sandbox, a hosted
playground — it cannot: providers refuse to render their consent screen inside an
iframe, so the flow has to run in a top-level popup and the code has to be handed
back to your page.

Two things are needed, and both are off by default:

1. List the embedding origin in
   [`security.frameAncestors`](/api-reference/modelence/server/type-aliases/SecurityConfig#frameancestors)
   — Modelence denies cross-origin framing by default, so the frame is blocked
   before OAuth matters.
2. Follow
   [Embedded previews and popups](/react-native#embedded-previews-and-popups)
   for the popup flow itself.
