...
```bash
npx create-modelence-app@latest ai-chat --template data-api
```
...
4. Copy the command that connects to remote DB, it should looks like the following:

```bash
npx modelence@latest setup --token "your token here"
```
