...
- **A Docker image built from the repository** with Node.js 18 or newer (default 22, set with
  ...
  preinstalled.
- **One web process**, started with the resource's `start`. It must listen on the port in the `PORT`
...
Modelence Cloud. Comments (`//`, `/* */`) and trailing commas are allowed. Every key is optional; a
missing key takes the default of the runtime.
...
| --- | --- | --- | --- |
| `$schema` | string | — | Set to `https://cloud.modelence.com/schema/modelence.config.json` for editor validation and completion. |
...
| Key | Type | Default | Meaning |
| --- | --- | --- | --- |
| `type` | `"node"` \| `"modelence"` | `"node"` | `"node"` runs any Node.js app exactly as described below. `"modelence"` is for apps built on the Modelence framework and fills in the framework's own build and start commands. |
| `build.node` | string | `"22"` | Node.js major version of the image, `"18"` or newer. |
| `build.root` | string | `"."` | Subdirectory that holds the app, relative to the repository root (monorepos). Commands run there, and `static` directories are relative to it. |
...
| `start` | string \| null | none | Command that starts the web process. `null` means there is no process: the site is static only and `static` must be set. |
| `static` | `[{ path, dir }]` | `[]` | Directories to serve. `path` is an absolute URL prefix (`"/"`, `"/docs"`); `dir` is relative to the build root and must exist after the build. |
...
| `scope` | `"runtime"` \| `"build-and-runtime"` | `"runtime"` | Which phases the variable reaches. `"runtime"` is the running app; `"build-and-runtime"` is the build steps **as well** — needed for anything a bundler inlines, such as Vite's `VITE_*`. |
...
dependency and keeps building them locally, as it always has. Add a `modelence.config.json` whose
resource has `"type": "modelence"` to move the build to Modelence Cloud instead.
</Note>

**The null / missing / empty rule** for `build.command` and `start`: a **missing** key inherits the
...
{
  "$schema": "https://cloud.modelence.com/schema/modelence.config.json",
  "resources": {
    "api": {
      "build": {
        "node": "22",
...
`/`; requests that match no file reach the API. `VITE_API_URL` is scoped `build-and-runtime` because
...
```json modelence.config.json
{
  ...
  "env": {
    "VITE_API_URL": { "value": "/api", "scope": "build-and-runtime" },
...
4. **Check the start script.** `vite`, `vite preview`, `react-scripts start`, `next dev`, `nodemon`,
   ...
   or, for a pure frontend, a static mount with `"start": null`.
...
7. **Write `modelence.config.json`** at the repository root with the values found above, as a single
   entry of `resources`. Set `build.root` when the app lives in a subdirectory. Add `"$schema"`.
...
bundler inlines `"scope": "build-and-runtime"` so it reaches the build as well as the running app.
