...
```
src/server/
├── migrations/       # (Optional) App migration scripts
│   ├── index.ts      # Exports the ordered migrations array
│   └── ...           # One file per migration handler
├── todo/
...
[Migrations](/core-concepts/migrations) for the full layout.
