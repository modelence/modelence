...
New Modelence projects declare the plugin in `.claude/settings.json`, which tells Claude Code where to
find it. Claude Code doesn't install plugins from external marketplaces on its own, so each person
installs it once per machine. How depends on where you run Claude Code:

<Tabs>
  <Tab title="Terminal">
    Start Claude Code in the project folder and, if asked, trust the folder. Claude Code notices the
    plugin isn't installed and shows the command to run:

    ```
    /plugin install modelence@modelence
    ```

    If it reports that the `modelence` marketplace isn't found, add it first and retry the install:

    ```
    /plugin marketplace add modelence/modelence
    ```

    The install summary says whether the plugin is already active. If it says to run
    `/reload-plugins`, run it — or start a new session.
  </Tab>
  <Tab title="VS Code / JetBrains">
    Open the project folder, open the Claude Code panel from the sidebar and, if asked, trust the
    folder. Type `/plugins` in the prompt box to open **Manage plugins**, find **Modelence** under the
    available plugins and click **Install**.

    If it isn't listed, switch to the **Marketplaces** tab, enter `modelence/modelence` to add the
    Modelence marketplace, then go back to the **Plugins** tab.

    After installing, a banner asks you to restart Claude Code — accept it. If nothing changes, reload
    the window from the Command Palette with **Developer: Reload Window**.
  </Tab>
  <Tab title="Claude Desktop">
    In the **Code** tab, start a session in the project folder. Click **+** next to the prompt box,
    then **Plugins** → **Add plugin**, find **Modelence** and install it.

    If it isn't listed, the Modelence marketplace hasn't been added on this machine yet. Open the
    session's terminal (from the **Views** menu, or with Ctrl+`) and run:

    ```
    claude plugin marketplace add modelence/modelence
    ```

    then open **Add plugin** again. This needs the [Claude Code CLI](https://code.claude.com/docs/en/setup)
    installed. Once installed, start a new session so the plugin loads; if it still doesn't appear under
    **Plugins**, quit Claude completely and reopen it.
  </Tab>
</Tabs>
...
with your Modelence account. Until then Claude Code lists the server as needing authentication and its
tools are unavailable; the rest of the plugin works regardless.

<Tabs>
  <Tab title="Terminal, VS Code, JetBrains">
    Run `/mcp`, choose `modelence`, then **Authenticate**. Your browser opens — sign in and choose the
    organization your app belongs to. Back in Claude Code the server shows as connected; no restart is
    needed.
  </Tab>
  <Tab title="Claude Desktop">
    Click **+** next to the prompt box, then **Plugins** → **Modelence**, and click **Connect** next
    to the `modelence` server. Your browser opens — sign in and choose the organization your app
    belongs to. If the panel still shows the server as not connected afterwards, start a new session.
  </Tab>
</Tabs>

Claude Code stores the sign-in per server, so it covers every Modelence project on your machine. See
...
This is the same file new projects ship with. Committing it means everyone on the team is prompted to
install the plugin when they open the project, and the setting stays project-scoped — it doesn't
