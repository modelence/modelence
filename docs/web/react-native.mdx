...
<Warning>
  ...
  Modelence logs a distinct console message naming COOP when it detects this,
  rather than failing with a generic "sign in again". Top-level (non-iframe)
  sign-in is unaffected, since there the verifier is read from the page's own
  `sessionStorage`.
</Warning>
